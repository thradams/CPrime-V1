...
void InstanciateDestroy(TProgram* program,
	...
	bool bInitExpressionIsAutoPointer,  //
...
void InstanciateDestroy(TProgram* program,
	...
	const char* pInitExpressionText, //(x->p->i = 0)
	bool bInitExpressionIsPointer,  //true quando o tipo da expressão eh um ponteiro para algo
	bool bInitExpressionIsAutoPointer,//true  quando o tipo da expressão eh um ponteiro para algo
	bool bSearchForInitFunction,    //true= procurar por funcao init
	StrBuilder* fp)
{
	...
	if (pMainSpecifier->Type == TSingleTypeSpecifier_ID)
	{
		...
		{
			if (bInitExpressionIsAutoPointer)
			{
				if (bInitExpressionIsPointer)
				{
	...
	else if (pMainSpecifier->Type == TStructUnionSpecifier_ID)
	{
		...
		if (bSearchForInitFunction)
		{
			...
			{
				//TODO se for ponteiro primeiro tem que procurar por
				//delete se nao achar procurar destroy e depois fazer free

				char funcName[200] = { 0 };
				strcat(funcName, pStructUnionSpecifier->Name);
				if (bInitExpressionIsPointer && bInitExpressionIsAutoPointer)
				{
					strcat(funcName, "_Delete");
				}
				else
				{
					strcat(funcName, "_Destroy");
				...
				if (pStructDeclaration != NULL)
				{
					...
					while (pStructDeclarator)
					{
						...
						InstanciateDestroy(program,
							...
							TDeclarator_IsAutoPointer(pStructDeclarator->pDeclarator),
							true,
							fp);
