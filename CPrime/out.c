  ...
  struct PPToken* close)
{
  ...
  for (int i = 0; i < formal_args->Size; i++)
  {
    ...
    Free(close->Lexeme);
    close->Lexeme = String_Create(t.Lexeme);
  }
...
bool Map_Set(Map* map, const char* key, void* data)
{
    ...
    //assert(key != NULL);
    String* /*@auto*/ localkey = String_Create(key);
...
bool MultiMap_Add(MultiMap* map, const char* key, void* data)
{
  ...
  if (result == true)
  {
    ...
    String * /*@auto*/stemp = String_Create(key);
  ...
  *ppPreviousValue = NULL;
  ...
  if (pMap->pHashTable != NULL)
  {
    ...
    if (pKeyValue == NULL)
    {
      ...
      pKeyValue->Key = String_Create(Key);
...
Tokens Parser_MatchEx(Parser* parser, TScannerItemList* listOpt, bool bKeepComment)
{
	...
	if (!Parser_HasError(parser))
	{
				...
				IsPreprocessorTokenPhase(token)))
		{
			...
			{
				//para ser ativado em pontos criticos para adicioanr comentario
				//porem como comentarios estao sendo usado como codigo...
				if (bKeepComment && token == TK_COMMENT)
				{
					//so adiciona os espacos no cara atual
					ScannerItem* pNew = ScannerItem_Create();
					LocalStrBuilder_Set(&pNew->lexeme, Scanner_LexemeAt(&parser->Scanner, 0));
        ...
        if (preToken == TK_PRE_INCLUDE)
        {
            ...
            if (IsIncludeState(state))
            {
                ...
                if (token == TK_STRING_LITERAL)
                {
                    ...
                    fileName = String_Create(lexeme + 1);
                        ...
                        const char* fullPath)
{
  ...
  pStream->NameOrFullPath = String_Create(fullPath);
...
bool Stream_Init(struct Stream* pStream, const char* name, const char*  Text)
{
  ...
  pStream->Column = 1;
  pStream->Position = 0;
  pStream->Text = String_Create(Text);
  pStream->NameOrFullPath = String_Create(name);
  pStream->FullDir2 = String_Create("");
...
char* String_Create(const char* p)
{
  char* a = 0;
  if (p)
  {
...
void MarkAsIncluded(const char* filename)
{
  ...
  pNew->fileName = String_Create(filename);
  pNew->pNext = 0;
...
WriteResult Write(char* fileNameRelative, bool bIsSystemHeader, FILE* out)
{
  ...
  {
    fullPath = String_Create(fileNameRelative);
  }
