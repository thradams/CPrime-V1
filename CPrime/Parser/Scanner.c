...
static bool GetNewMacroCallString(Scanner* pScanner,
                                  BasicScanner* pBasicScanner, Macro* pMacro,
                                  TokenArray* ppTokenArray,
                                  StrBuilder* strBuilder,
                                  bool bIAlreadyHaveMacroName,
                                  TScannerItemList* pAcumulatedTokensLocal)
{
  // StrBuilder_Append(strBuilderResult, Scanner_LexemeAt(pScanner));
  // TODO aqui nao pode ser o current
  const char* lexeme = pBasicScanner->currentItem.lexeme.c_str;
  ...
  if (nArgsExpected > 0)
  {
    ...
    if (!bIAlreadyHaveMacroName)
    {
      ...
      pNew->token = token;
      pNew->bActive = true;
      List_Add(pAcumulatedTokensLocal, pNew);
      ////////////
    ...
    {
      StrBuilder_Append(&macroName, pMacro->Name);
    }
    ...
    if (token == TK_LEFT_PARENTHESIS)
    {
      // Adiciona o nome da macro
      PPToken* ppTokenName =
        PPToken_Create(macroName.c_str, PPTokenType_Identifier);
      TokenArray_Push(ppTokenArray, ppTokenName);
